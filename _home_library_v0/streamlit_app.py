...
# Stramlit이 이 주소로 HTTP 요청을 보내서 데이터를 주고 받는다.
# (FastAPI와 Stramlit은 완전히 다른 프로그램 두 개가 따로 실행되는 것 --> 서로 requests로 대화하는 구조)
...
#     r = requests.post(
#         f'{API}/books/scan',
#         files={'images':(image.name, image.getvalue(), image.type)}
#     )
