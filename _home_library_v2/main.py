...
def _deplicate_detail(existing_book: Book) -> dict:
    """
    중복된 책을 만났을 때 응답에 실을 정보를 한 곳에서 만든다.
    look_book, register_book 두 군데에서 똑같이 재사용하기 위한 헬퍼 함수
    """
    return {
        'message': '이미 등록된 책입니다.',
        'existing_book': {
            ...
            'recognition': existing_book.recognition_status,
        },
    }
...
def lookup_book(isbn: str, db: Session = Depends(get_db)):
    ...
    if existing_book: # 중복된 책이 있다면 409에러 발생 -> _deplicate_detail 헬퍼 함수 호출
        raise HTTPException(status.HTTP_409_CONFLICT, _deplicate_detail(existing_book))
