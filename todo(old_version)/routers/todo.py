...
# - main.py 에서 app.include_router(todo_router)로 등록되어야 실제로 동작한다.

# 라우터(router)??
# - 웹 애플이케이션에서 여러 API를 기능별로 구분해 한곳에 모아두기 위한 단위
...
def get_todos_headler():
    ...
    try: # stmt : SQL문을 의미하는 statement의 약자
        ...
        todos = session.execute(stmt).scalars().all() # 전체 결과 리스토로 반환
        return todos
    finally:  # 예외가 발생하든 무조건 실행
        session.close()
