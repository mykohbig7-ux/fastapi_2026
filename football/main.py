...
#   --> Swagger UI(/docs)와 Redoc(/redoc)의 상단 섦여 영역에 들어간다.
...
## 분석(Analytics)
API의 상대와 리그, 팀, 선수 수에 대한 정보를 제공합니다.
...
## 멤버십(Membership)
SWC 판타지 풋볼리그 전체와 각 리그에 속한 팀에 대한 정보를 제공합니다.
'''
app = FastAPI(
...
def get_db():
    ...
    db = SessionLocal()
    try:
        # yieid 앞은 요청 처리 전에 실행되고, yield로 넘김 db가 엔드포인트 함수에 주입된다
        yield db 
    finally:
...
@app.get('/v0/players/{player_id}', response_model=schemas.Player,
        ...
        description='다른 API 호출(v0_get_player 등)에서 얻은 SWC 선수 ID를 이용해
        해당 선수를 조회할 수 있습니다.''',
...
def read_player(player_id: int,
                db: Session = Depends(get_db)):
    """player_id 하나로 특정 선수 상세 정보 조회한다."""
...
@app.get('/v0/teams/', response_model=list[schemas.Team],
        ...
        skip과 limit를 사용해 페이지을 수행합니다. 팀 이름은 고유하지 않습니다.
...
def read_teams(skip: int = Query(0, description='API 호출 시작 부분에서 건너뛸 항목 수입니다.'), 
              limit: int = Query(100, description='건너뛴 레코드 이후 반환할 레코드 수입니다.'), 
              minimum_last_changed_date: date= Query(None, description='변경 기준 날짜입니다. 이 날짜 이후에 변경된 레코드만 반환합니다.'),
              team_name: str = Query(None, description='반환할 팀 이름입니다. SWC 전체에서 고유하지 않으나, 리그 내부에서는 고유합니다.'),
