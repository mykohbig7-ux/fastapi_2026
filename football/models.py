...
class Player(Base):
    ...
    #   ex) Player.performance 라고 쓰면 이 선수의 Performance 객체 목록을 
    ...
    # ----------------------------------------------------------------------------------
    # N:M 관계 (선수 여러명 <-> 팀 여러개) - 다대다 -> Asscoiation Object 패턴
...
class TeamPlayer(Base):
    ...
    team_id: Mapped[int] = mapped_column( # team 테이블 안 team_id
        ForeignKey('team.team_id'), primary_key=True, index=True
    )
    player_id: Mapped[int] = mapped_column( # player 테이블 안 player_id
