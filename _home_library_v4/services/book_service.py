...
새로 추가되는 HTML 라우터(/ui/books/lookup, /ul/books/register)
...
import uuid # 랜덤 문자열을 생성해주는 라이브러리 (중복되는 파일이 안 생기도록 해준다.)
...
class ServiceResult:
    ...
    API 라우터 --> 실패 시 HTTPEXception 에러 상태 코드로 응답한다.
    HTML 라우터--> 실패 시 에러 화면이 아니라, "이미 등록된 책입니다"와 같은 안내 문구가 나와야 한다.
