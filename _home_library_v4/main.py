...
# Jinja2Templates --> templates 폴더 안의 .html파일에 데이터를 채워서 완성된 HTML 페이지로 만들어준다.
...
#   - 나중에 마운트 경로가 바뀌어도 템플릿을 일일이 안 고쳐도 되게 하기 위해서
# -------------------------------------------------------------------------
...
def shelf(request: Request, db: Session=Depends(get_db)):
    ...
    # order_by(Book.created_at.desc()) --> 등록된 시간(created_at) 기준으로 최신순(내림차순) 정렬
